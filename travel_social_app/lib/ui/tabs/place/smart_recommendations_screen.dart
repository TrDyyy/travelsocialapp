...
import 'package:travel_social_app/ui/smart_recommendation/widgets/select_favorite_types_page.dart';
import 'package:travel_social_app/ui/tabs/place/widgets/place_detail_sheet.dart';
...
class _SmartRecommendationsScreenState extends State<SmartRecommendationsScreen>
  ...
  Future<void> _loadSmartRecommendations() async {
    if (mounted) setState(() => _isLoadingSmart = true);
    try {
  ...
  Future<void> _loadNearbyRecommendations() async {
    if (mounted) setState(() => _isLoadingNearby = true);
    try {
      final recommendations = await _recommendationService
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Lỗi tải địa điểm gần: $e')));
      }
    }
  }

  Future<void> _loadPreferenceRecommendations() async {
    if (mounted) setState(() => _isLoadingPreference = true);
    try {
      final recommendations = await _recommendationService
